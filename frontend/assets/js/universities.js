// Render universities to grid
function renderUniversities() {
  const container = document.getElementById('universitiesGrid');
  const noResults = document.querySelector('#noResults'); // Should handle both ID/class conventions
  const countSpan = document.getElementById('resultsCount');

  if (!container) return;

  // Update count
  if (countSpan) countSpan.textContent = filteredUniversities.length;
  ...
  container.innerHTML = filteredUniversities.map(uni => `
      <div class="university-card">
        <div class="uni-image-wrapper">
          <span class="uni-ranking-badge">#${uni.ranking}</span>
          <img src="${uni.logo || '/assets/images/university-placeholder.png'}" alt="${uni.name}" class="uni-logo" onerror="this.src='https://via.placeholder.com/150?text=UNI'">
        </div>
          ...
          <div class="uni-header">
            <div>
              ...
              <div class="uni-location">
                <i class="fas fa-map-marker-alt"></i> ${uni.city}, ${uni.country}
              </div>
            </div>
            <button class="btn-save-uni ${isFavorite(uni.id) ? 'active' : ''}" onclick="window.toggleFavorite('${uni.id}')">
              <i class="${isFavorite(uni.id) ? 'fas' : 'far'} fa-heart"></i>
            </button>
          </div>

          <p class="uni-description">
            A leading institution known for excellence in ${uni.majors && uni.majors.length > 0 ? uni.majors[0] : 'research'} and innovation. 
            Home to world-class facilities and a diverse student community.
          </p>

          <div class="uni-stats">
            <div class="uni-stat-item">
              <i class="fas fa-sack-dollar"></i>
              <span>$${(uni.tuition / 1000).toFixed(1)}k/yr</span>
            </div>
            <div class="uni-stat-item">
              <i class="fas fa-check-circle"></i>
              <span>${uni.acceptanceRate}% Accept</span>
            </div>
             <div class="uni-stat-item">
              <i class="fas fa-user-graduate"></i>
              <span>${uni.studentCount ? (uni.studentCount / 1000).toFixed(1) + 'k' : '25k+'}</span>
            </div>
          </div>

          <div class="uni-footer">
            <div class="uni-tags">
              ${uni.majors ? uni.majors.slice(0, 2).map(m => `<span class="uni-tag">${m}</span>`).join('') : ''}
              ${uni.type ? `<span class="uni-tag">${uni.type}</span>` : ''}
            </div>
            
            <div class="uni-actions">
              <button class="btn-uni-outline" onclick="window.viewDetails('${uni.id}')">View Details</button>
              <button class="btn-uni-primary">Apply Now</button>
            </div>
          </div>
        </div>
      </div>
...
window.toggleFavorite = function (id) {
  if (toggleFavoriteLocal(id)) { // Call internal function
    renderUniversities();
  }
};

// Internal toggle helper that was defined in original file (or needs to be)
function toggleFavoriteLocal(id) {
  ...
  try {
    localStorage.setItem('brainex_university_favorites', JSON.stringify(favorites));
    return true;
  } catch (e) { console.error(e); return false; }
}

// Ensure view details is global
window.viewDetails = function (id) {
  // Find uni
  const uni = universities.find(u => u.id == id);
  if (uni) {
    alert('Details modal for ' + uni.name + ' coming soon!');
  }
};
...
function renderUniversities() {
  ...
  elements.grid.querySelectorAll('.btn-favorite').forEach((btn) => {
    btn.addEventListener('click', (e) => {
      e.stopPropagation(); // Prevent card click
      const id = e.target.closest('.btn-favorite').dataset.id;
      toggleFavorite(id);
    });
  });

  // Make entire card clickable to show detail
  elements.grid.querySelectorAll('.university-card').forEach((card) => {
...
function createUniversityCard(uni) {
  ...
  return `
            <article class="university-card" data-id="${uni.id}">
                <button class="btn-favorite" data-id="${uni.id}" title="${isFav ? 'Remove from favorites' : 'Add to favorites'}" style="position: absolute; top: 1rem; right: 1rem; background: ${isFav ? '#f56565' : 'rgba(255,255,255,0.9)'}; border: none; border-radius: 50%; width: 36px; height: 36px; cursor: pointer; font-size: 18px; transition: all 0.2s; z-index: 2; box-shadow: 0 2px 8px rgba(0,0,0,0.15);">
                    ${isFav ? '❤️' : '🤍'}
                </button>
                ...
                <div class="card-actions">
                    <a href="${uni.website}" target="_blank" rel="noopener noreferrer" class="btn-view">Visit Website</a>
...
function showUniversityDetail(id) {
  const uni = universities.find((u) => u.id == id); // Use == for loose comparison
  if (!uni) return;
