...
class UniversitiesEngine {
    ...
    async init() {
        try {
            // Load Data
            const response = await fetch('/frontend/data/universities.json');
            ...
            if (this.container) this.container.innerHTML = '<div class="error-state">Failed to load data. Please try again later.</div>';
        }
    }
    ...
    setupFilters() {
        ...
        if (tuitionInput) {
            tuitionInput.addEventListener('input', (e) => {
                ...
                if (tuitionDisplay) tuitionDisplay.textContent = `$${this.filters.maxTuition.toLocaleString()}`;
                this.applyFilters();
            });
        }
    ...
    applyFilters() {
        this.filteredData = this.data.filter(uni => {
            const matchesSearch = uni.name.toLowerCase().includes(this.filters.search) ||
                uni.programs.some(p => p.toLowerCase().includes(this.filters.search));
            const matchesCountry = this.filters.country === '' || uni.location.country === this.filters.country;
    ...
    render() {
        ...
        if (this.filteredData.length === 0) {
            this.container.innerHTML = '<div class="empty-state">No universities found matching your criteria.</div>';
            return;
        }

        this.container.innerHTML = this.filteredData.map(uni => `
            <div class="uni-card animate-fade-in">
                ...
                <div class="uni-body">
                    ...
                    <div class="uni-tags">
                        ${uni.programs.slice(0, 3).map(p => `<span class="uni-tag">${p}</span>`).join('')}
                    </div>
                </div>
