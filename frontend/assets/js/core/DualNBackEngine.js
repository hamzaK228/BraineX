...
export class DualNBackEngine {
    ...
    nextRound() {
        ...
        if (shouldMatchPosition) {
            ...
            } while (this.history.length >= this.n && position === this.history[this.history.length - this.n].position && Math.random() > 0.1);
        }
        ...
        if (shouldMatchAudio) {
            ...
            } while (this.history.length >= this.n && letter === this.history[this.history.length - this.n].letter && Math.random() > 0.1);
        }
