...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.category-card').forEach((card) => {
    card.addEventListener('click', (e) => {
      // Don't trigger if clicking the button (button handles it)
...
function renderFields(fields) {
  ...
  container.innerHTML = fields
    .map(
      (field) => `
        <div class="field-card" data-category="${field.category || 'other'}" onclick="window.location.href='/fields.html?id=${field.id}'">
            <div class="field-icon-wrapper">
                <span class="field-icon" style="font-size: 3.5rem;">${field.icon || '🎓'}</span>
            </div>
            <h4>${escapeHtml(field.name)}</h4>
            <p>${escapeHtml(field.description || 'Explore this exciting academic field and discover related career paths and opportunities.')}</p>
            
            <div class="field-stats-row">
                <div class="card-stat">
                    <i class="fas fa-briefcase"></i>
                    <span>${escapeHtml(field.avg_salary || '$80k+')}/yr</span>
                </div>
                <div class="card-stat">
                    <i class="fas fa-chart-line"></i>
                    <span>${escapeHtml(field.growth_rate || '15%')} Growth</span>
                </div>
            </div>

            <div class="field-tags">
                ${(field.tags || ['General']).slice(0, 3).map(tag => `<span class="card-tag">${escapeHtml(tag)}</span>`).join('')}
            </div>

            <button class="btn-card-explore btn-explore-field" data-id="${field.id}">
                Explore Field <i class="fas fa-arrow-right" style="margin-left: 5px;"></i>
            </button>
        </div>
    `
    )
