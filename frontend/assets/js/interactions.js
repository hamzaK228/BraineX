...
(function () {
    ...
    function initScrollAnimations() {
        ...
        const animatableElements = document.querySelectorAll('.card, .stat-item, .feature-card, .program-card, .university-card, h2, h3:not(.no-anim)');
    ...
    function initAccessibility() {
        ...
        document.querySelectorAll('button:not([aria-label])').forEach(btn => {
            if (btn.innerText.trim().length === 0 || btn.querySelector('img, svg')) {
                ...
                if (btn.classList.contains('mobile-menu-btn')) btn.setAttribute('aria-label', 'Toggle Menu');
                else if (btn.classList.contains('theme-toggle')) btn.setAttribute('aria-label', 'Toggle Theme');
