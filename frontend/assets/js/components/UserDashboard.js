...
export class UserDashboard extends Component {
    ...
    renderRecentBadges(unlockedIds) {
        ...
        return `
            ...
            return badge ? `<div title="${badge.name}" style="background: rgba(255,255,255,0.2); width: 32px; height: 32px; border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: help;">${badge.icon}</div>` : '';
        }).join('')}
                </div>
            </div>
        `;
